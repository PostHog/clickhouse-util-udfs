...
func TestProcessLineRemovesDuplicateKeys(t *testing.T) {
	tests := map[string]string{
		`{"id":1,"a":null,"a":"x","b":""}`:                 `{"id":1,"a":"x","b":""}`,
		`{"id":2,"a":"","a":""}`:                           `{"id":2,"a":""}`,
		`{"id":5,"a":1,"a":2,"b":true,"b":false}`:          `{"id":5,"a":1,"b":true}`,
		`{"id":8,"obj":{"x":""},"obj.x":"y"}`:              `{"id":8,"obj":{"x":"y"}}`,
		`{"id":10,"amount":934504962295726700000}`:         `{"id":10,"amount":"934504962295726700000"}`,
		`{"id":11,"a":[{"k":"","k":"x"},{"k":"","k":""}]}`: `{"id":11,"a":[{"k":"x"},{"k":""}]}`,
	}
...
func BenchmarkProcessLine(b *testing.B) {
	input := []byte(`{"id":1,"a":null,"a":"x","b":"","nested":{"k":"","k":"v"},"arr":[{"k":"","k":"x"},{"k":"","k":""}],"amount":934504962295726700000}`)
	var buf bytes.Buffer
...
func generatedBenchmarkLines() ([][]byte, int) {
	...
	for i := 0; i < 256; i++ {
		line := []byte(fmt.Sprintf(
			`{"id":%d,"a":null,"a":"value-%d","b":"","nested":{"k":"","k":"v%d"},"arr":[{"k":"","k":"x%d"},{"k":"","k":""}],"amount":934504962295726700000}`,
			i,
			i,
			i,
			i,
